...
import pandas as pd
import time
from carmaker_cone import Cone, Car, Road
...
class CarMakerEnv(gymnasium.Env):
    def __init__(self, check=2, port=10001, simul_path='pythonCtrl_JX1', road_type="DLC"):
    ...
    def step(self, action1):
        ...
        if state == False:
            ...
            time = state[1]
            carx, cary, caryaw, car_v = state[2:6] #x, y, yaw
            car_steer = state[6:9]
            dev = self.calculate_dev(carx, cary, caryaw)
            alHori, roll = state[9:11]
            wheel_steer = state[11:15]
            r_ext = state[15:]
            ...
            ahead_cones = self.road.cone.arr[self.road.cone.arr[:, 0] > carx][:1]
            behind_cones = self.road.cone.arr[self.road.cone.arr[:, 0] <= carx][:1]
            closest_cones = np.vstack((behind_cones, ahead_cones))
            cones_rel = self.to_relative_coordinates(carx, cary, caryaw, closest_cones).flatten()
            state = np.concatenate((dev, np.array([caryaw, car_v, car_steer[0], car_steer[1]]), wheel_steer, r_ext, lookahead_traj_rel, cones_rel))
    ...
    def getReward(self, state, time):
        ...
        col_reward = self.is_car_in_lane(carx, cary, caryaw) * 1000
    ...
    def is_car_in_lane(self, carx, cary, caryaw):
        car_shape = self.car.shape_car(carx, cary, caryaw)
        if self.road.lane.boundary_shape.contains(car_shape):
            return 0
        return 1
