...
ROADLENGTH, ROADWIDTH = 500, -15,
CARLENGTH, CARWIDTH = 4.3, 1.568
...
class Road:
    def __init__(self):
        self.road_length = ROADLENGTH
        self.road_width = ROADWIDTH
...
#        print(f'reset : {self.reset_num}')
    ...
    def step(self, action):
        ...
        cones_sight = self.cone_in_sight(self.car.carx, 2)
        middles_sight = self.middle_in_sight(self.car.carx, 2)
        ...
        if self.road.is_car_in_road(car_shape) == 1:
            done = True
        reward = self.getReward(car_shape)
    ...
    def getReward(self, car_shape):
        ...
        if self.car.carx >= 370 or self.car.carx <= 85:
    ...
    def is_car_in_road(self, car_shape):
        if not car_shape.intersects(self.road.road_boundary):
            return 1
        if not self.road.road_boundary.contains(car_shape):
            return 1
        return 0
