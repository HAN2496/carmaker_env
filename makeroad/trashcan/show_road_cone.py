...
class Cone:
    def __init__(self, x, y, offset, num_cones, gap, section):
        self.section = section
        self.positions = self._create_cones(x, y, offset, num_cones, gap)

    def _create_cones(self, x, y, offset, num_cones, gap):
        positions = []
        internal_pos = 0
        for _ in range(num_cones):
            if offset == 0:
                positions.append((x, y, internal_pos))
                internal_pos += 1
                x += gap
            else:
                positions.append((x, y - offset, internal_pos))
                internal_pos += 1
                positions.append((x, y + offset, internal_pos))
                internal_pos += 1
                x += gap
