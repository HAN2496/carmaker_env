...
class CMcontrolNode:
    def __init__(self, action_queue, state_queue, action_num, state_num,
                 host='127.0.0.1', port=10001, matlab_path='C:/CM_Projects/JX1_102/src_cm4sl', simul_path='JX1_RWS_project_v2_simple2'):
    ...
    def _setup_sim(self):
        ...
        # 통신을 위한 포트 지정
        print("set_param('{}/CarMaker/tcpiprcv', 'Port', '".format(self.SIMUL_PATH)+str(self.port)+"')")
        self.eng.set_param('{}/CarMaker/tcpiprcv'.format(self.SIMUL_PATH), 'Port', str(self.port), nargout=0)
        print("set_param({}'/CarMaker/tcpipsend', 'Port', '".format(self.SIMUL_PATH) + str(self.port)+"')")
        self.eng.set_param('{}/CarMaker/tcpipsend'.format(self.SIMUL_PATH), 'Port', str(self.port), nargout=0)
...
if __name__ == "__main__":
    ...
    def cm_thread(port, action_queue, state_queue):
        cm_env = CMcontrolNode(port=port, action_queue=action_queue, state_queue=state_queue, action_num=2, state_num=13)
        cm_env.start_sim()
        cm_env.stop_sim()
