...
from stable_baselines3.common.MyBuffer import CustomReplayBuffer
...
def main():
    ...
    pretrain_steps = 50
    learn_steps = 100
    comment = f"various_expert_buffer{buffer_size}k_pretrain{pretrain_steps}k_learn{learn_steps}k"
    ...
    replay_buffer2 = ReplayBuffer(buffer_size=buffer_size, observation_space=env.observation_space,
                                 action_space=env.action_space, n_envs=num_proc)
    ...
    for idx in range(buffer_size):
        ...
        replay_buffer2.add(obs, next_obs, act, rew, done, info)


    # 모델의 리플레이 버퍼에 넣어주기
#    model.replay_buffer = replay_buffer
    #학습시작
    model.pretrain(pretrain_buffer=replay_buffer2, callback=bestRewardCallback, info_data = infos, pretrain_steps=pretrain_steps, total_timesteps=learn_steps)
    model.learn(total_timesteps=learn_steps, callback=bestRewardCallback, pretrain_steps=pretrain_steps)
