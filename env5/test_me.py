...
class MetaNN:
    def __init__(self, input_shape):
        with tf.device('/gpu:1'):
            ...
            x = Conv2D(32, kernel_size=3, strides=(1, 1), padding='same', data_format='channels_last',
...
class NN:
    def __init__(self, input_shape):
        with tf.device('/gpu:1'):
            ...
            z = Conv2D(8, kernel_size=5, strides=(1, 1), data_format='channels_last', activation='relu')(
                inputs_termination)
            z = Dropout(0.5)(z)
    ...
    def collect(self, processed, expert_a, goal, initiation_point):
        if processed is not None:
            self.replay_hist[self.ind] = (
            processed.astype(np.float32), expert_a.astype(np.float32), goal, initiation_point.astype(np.float32))
            self.ind = (self.ind + 1) % TRAIN_HIST_SIZE
            self.count += 1
    ...
    def train(self):
        ...
        self._train_x_difference = np.reshape(
            [self._selected_replay_data[i][0] - self._selected_replay_data[i][3] for i in xrange(BATCH_SIZE)],
            (BATCH_SIZE,) + self.input_shape)
...
class Agent(Visualizable):
    ...
    def collect_experience(self, agent_loc, prev_state, agent_action, reward, next_state, expert_advice):
        self.experience[self.ind] = (
        agent_loc, prev_state.astype(np.float32), agent_action, reward, next_state.astype(np.float32),
        expert_advice.astype(np.float32))
        self.ind = (self.ind + 1) % TRAIN_HIST_SIZE
    ...
    def get_expert_trajectory(self, agent_host, starting_loc, goal_loc):
        ...
        for index in range(len(self.room_trajectory)):
            if self.room_trajectory[index] == (-1, -1):
                ...
                change_point.append(
                    index + 1)  # the next index should be the terminal point of an option, and should be an intermediary location
        change_point.append(len(self.room_trajectory))  ## the last location

        print
        "length of room trajectory ", len(self.room_trajectory)
        ...
        for index in range(len(change_point) - 1):
            ...
            if end_index < len(self.room_trajectory):
                room_shift = (self.room_trajectory[end_index][0] - self.room_trajectory[begin_index][0],
                              self.room_trajectory[end_index][1] - self.room_trajectory[begin_index][1])
    ...
    def get_expert_feedback(self, agent_host, macro_action, expert_policy, decision_point):
        ...
        if agent_host._world[agent_loc[0], agent_loc[1]] == 'x':
            ...
            for action in range(4):
                ...
                if state[0] >= 0 and state[0] <= 16 and state[1] >= 0 and state[1] <= 16 and agent_host._world[
                    state[0], state[1]] == 'w':
            ...
            if macro_action == 0:
                if agent_loc[1] % 4 == 3 and agent_loc[1] < 15 and agent_loc[1] < decision_point[1] and (
                        agent_loc[0] / 4) == (decision_point[0] / 4):
                    expert_t = 1
                else:
                    expert_t = 0
            elif macro_action == 1:
                if agent_loc[1] % 4 == 1 and agent_loc[1] > 1 and agent_loc[1] > decision_point[1] and (
                        agent_loc[0] / 4) == (decision_point[0] / 4):
                    expert_t = 1
                else:
                    expert_t = 0
            elif macro_action == 2:
                if agent_loc[0] % 4 == 3 and agent_loc[0] < 15 and agent_loc[0] < decision_point[0] and (
                        agent_loc[1] / 4) == (decision_point[1] / 4):
                    expert_t = 1
                else:
                    expert_t = 0
            elif macro_action == 3:
                if agent_loc[0] % 4 == 1 and agent_loc[0] > 1 and agent_loc[0] > decision_point[0] and (
                        agent_loc[1] / 4) == (decision_point[1] / 4):
                    expert_t = 1
                else:
                    expert_t = 0
            ...
            if expert_t == 0 and macro_action != self.identify_optimal_macro_action(agent_host, agent_loc):
                print
                "not optimal meta action chosen"
                expert_a = 99
                expert_t = 99
        # else:
    ...
    def identify_optimal_macro_action(self, agent_host, starting_loc):
        if agent_host._world[starting_loc[0], starting_loc[1]] == 'x':
            print
            "recursive call to determine best meta action"
            possible_actions = [(0, -1), (0, 1), (-1, 0), (1, 0)]
            best_direction = (0, 0)
            max_value = -1.0
            for action in range(4):
            ...
            if starting_room == goal_room:
                ...
                if room_shift == (0, -1):
                    ...
                    print
                    "something is wrong when identifying optimal macro action"
    ...
    def learner_choose_option(self, agent_host):
        ...
        if tau >= 0.9:
            print
            "train - had to pick another option to prevent loops"
    ...
    def run_train(self, agent_host):
        ...
        if meta_controller_call_history.count(agent_host.agent_loc) >= 1:
            print
            "Already request meta label here at least once. We're not adding more"
        else:
        ...
        if self.warmstart or self._macro_a != expert_macro_a:
            if self.warmstart:
                print
                "warmstart phase - adding meta data"
                self.train_flag = True
            elif self._macro_a != expert_macro_a and self.rerun_count <= 10:
                print
                "option selected not optimal, hold the training flag"
            elif self._macro_a != expert_macro_a and self.rerun_count > 10:
                print
                "option selected not optimal, turn on training flag"
                self.train_flag = True
        ...
        while (not self.terminate) and (not agent_host.done) and self._step < HORIZON:
            ...
            experience = (
            agent_loc, self._state, self._macro_a, self.expert_macro_a, self._a, self._tau, self.expert_a, expert_t,
            self.initiation_point, self.decision_point)
            ...
            if self._tau < 0.9:
                ...
                if ((len(option_segment) >= 5)
                        and (option_segment[-1][0] == option_segment[-3][0]) and (
                                option_segment[-1][4] == option_segment[-3][4])
                        and (option_segment[-1][0] == option_segment[-5][0]) and (
                                option_segment[-1][4] == option_segment[-5][4])):  # same spot and same action taken
                    print
                    print
                    "looks like getting in a loop, send a break signal"
                    self.train_flag = True
                    self.terminate = True
                # else:
        ...
        if agent_host.done:
            ...
            final_experience = (
            agent_host.agent_loc, self._state, self._macro_a, self.expert_macro_a, self._a, 0.0, self.expert_a, 0.0,
            self.initiation_point, self.decision_point)  ## intentionally avoid 1 for tau, for goal
        ...
        print
        "number of segments:", len(self.trajectory)
        print
        "actual number of meta labels to be collected:", len(meta_controller_call_history)
        # time.sleep(0.5)
        if self.warmstart:
            ...
            if self.train_flag:
                ...
                for index in range(len(self.trajectory)):
                    ...
                    assert option_index == option_segment[0][
                        2]  ## make sure the options executed in the segment is consistent
                    ...
                    assert decision_point == option_segment[0][
                        9]  ## make sure the initiation point stays consistent throughout the segment
                    # print
                    ...
                    if option_index == np.argmax(option_segment[0][3][
                                                     0]):  # if the option executed is the same as the optimal option at the beginning of the segment, then it is good
                        print
                        "Good option ", option_index, " being executed"
                        ...
                        if not goal_achieved:
                            print
                            "Goal not achieved for meta action:", option_index
                            label_count = 0
                            # self.ind = self.ind + len(option_segment)
                            for item in option_segment:
                                ...
                                if sum(item[6][0]) < 1:
                                    print
                                    "expert advice for this is:", item[6][0], "so not collecting"
                                else:
                                    self.model[item[2]].collect(item[1], item[6], item[7], item[8])
                                    self.ind += 1
                                    label_count += 1
                            print
                            "Added ", label_count, " labels for training option ", option_index
                            if label_count > 0:
                                self.ind = self.ind - 1  # take one back to avoid double counting the meta actions
                    else:
                        print
                        "bad option ", option_index, " attempted. Will ignore"
        if agent_host._rendering:
            time.sleep(0.1)
    ...
    def choose_option_test(self, agent_host):
        ...
        if tau >= 0.9:
            print
            "test - had to pick another option to prevent loops"
            self._macro_action_pred[self._macro_a] = 0.0  # disable this option
            resum = sum(self._macro_action_pred) * 1.0
            self._macro_action_pred = self._macro_action_pred / resum
            self._macro_a = self.sample(self._macro_action_pred)
    ...
    def run_test(self, agent_host):
        ...
        print
        print
        "total number of options called:", self._option_step
    ...
    def inject_summaries(self, idx):
        if self.mode == 'train':
            ...
            if len(self._stats_success) > 100:
                ...
                self.visualize(self.teach_episode, "adjusted episode success indicator",
                               np.asscalar(np.mean(self._stats_success[-100:])))
                # self.visualize(self.ind+self.meta_ind, "learning curve",np.asscalar(np.mean(self._stats_success[-100:])))
                self.visualize(self.ind + self.meta_ind, "learning curve",
                               np.asscalar(np.mean(self._stats_success[-100:])))
            else:
...
def main(macro_action, train_or_test, environment, validation):
    ...
    agent_host = MazeNavigationEnvironment(MazeNavigationStateBuilder(gray=False),
                                           rendering=False, randomized_door=True, stochastic_dynamic=False, map_id=999,
                                           setting=environment)
    ...
    for i in six.moves.range(1, NUM_EPISODES + 1):
        ...
        if agent.warmstart:
            ...
            final_reward = agent.run_train(agent_host)
            print
            "final reward for this training round:", final_reward
            while agent.rerun:
                print
                print
                "********rerun since didnt learn anything last run****"
                # agent.train_buffer += 1
                print
                print
                "re-attempt number:", agent.rerun_count
                new_map_id = np.random.choice(range(1000))
                agent_host.change_map_and_reset(new_map_id)
        ...
        if i % SAVE_MODEL_EVERY == 0:  # throw away the first 1000 episodes?
            print
            "to save"
            agent.save_success_record(
                'summary/success_record/result_' + SUMMARY_NAME + '_' + str(int(i / SAVE_MODEL_EVERY)) + '.pkl')
