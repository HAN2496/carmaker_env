...
class CarMakerEnv(gym.Env):
    def __init__(self, host='127.0.0.1', port=10001, check=2, matlab_path='C:/CM_Projects/PTC0910/src_cm4sl', simul_path='pythonCtrl_Straight'):
        ...
        self.ipg_data = pd.read_csv(f"datafiles/{self.road_type}/IPG_Straight_info.csv").iloc[:-1]
        self.ipg_x = self.ipg_data["carx"].values
        self.ipg_y = self.ipg_data["cary"].values

        self.traj_data = pd.read_csv(f"datafiles/{self.road_type}/datasets_traj_Straight_1.csv").loc[:, ["traj_tx", "traj_ty"]].values
