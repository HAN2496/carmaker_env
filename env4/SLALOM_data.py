...
class Data:
    ...
    def to_relative_coordinates(self, arr):
        ...
        for point in arr:
            ...
            rotated_x = dx * np.cos(-self.yaw) - dy * np.sin(-self.yaw)
            rotated_y = dx * np.sin(-self.yaw) + dy * np.cos(-self.yaw)
    ...
    def mangae_state_for_low(self, state):
        ...
        reward_argument = {"devDist": self.devDist, "devAng":self.devAng}
    ...
    def manage_state_for_b(self, steering_changes, arr, action):
        ...
        yawrate = (self.yaw - self.yaw_before) / 0.01

        state = np.concatenate((steering_changes, np.array([self.steerAng, self.steerVel, self.yaw, yawrate, self.cary + 10]), traj_point_new_rel, cones_rel)) # <- Policy B의 state
        reward_argument = {"traj": traj_point_new, "carx": self.carx, "cary": self.cary, "yaw": self.yaw}
        info_key = np.array(["time", "x", "y", "yaw", "carv", "ang", "vel", "acc", "devDist", "devAng", "alHori", "roll", "rl", "rr", "fl", "fr"])
        info = {key: value for key, value in zip(info_key, arr[1:])}

        self.yaw_before = self.yaw
    ...
    def _init_reward_argument(self):
        return {"traj": self.traj_point, "carx": self.carx, "cary": self.cary, "yaw": self.yaw}
    ...
    def render(self, mode='human'):
        ...
        rotated_corners = []
        for x, y in corners:
            x_rot = x * np.cos(-self.yaw) - y * np.sin(-self.yaw) + self.carx * XSIZE
            y_rot = x * np.sin(-self.yaw) + y * np.cos(-self.yaw) - self.cary * YSIZE
